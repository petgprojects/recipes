...
export const recipes = pgTable(
  'recipes',
  {
    ...
    /**
     * The public link key: eight characters, generated by `gen_share_code()`
     * (migration 0005). A share link is `/r/<slug>-<share_code>` and resolves
     * on this alone — the slug in front is decoration, which is what lets a
     * pasted link survive a Phase 2 title rewrite.
     *
     * Defaulted in the database rather than assigned by the writer: every
     * insert path (JSON-LD, the LLM extractor, the Reddit adapter) then gets a
     * code without knowing this column exists.
     */
    shareCode: text('share_code')
      .notNull()
      .default(sql`gen_share_code()`),
  ...
  (t) => [
    ...
    // The share-link lookup, and the guarantee the link format rests on.
    uniqueIndex('recipes_share_code_key').on(t.shareCode),
    ...
    // The shape `parseRecipeHandle()` in @recipes/shared/share parses against,
    // enforced where it cannot drift from the parser.
    check('recipes_share_code_shape', sql`${t.shareCode} ~ '^[0-9a-hjkmnp-tv-z]{8}$'`),
